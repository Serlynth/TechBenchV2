...
Current milestone: `0.5.38` - TechBench V2 uses the `0.5.x` development release line; "V2" remains the product generation and is assumed. Credentials workbook columns are discovered from headers and stored as flexible encrypted fields. Client Credentials clipboard operations run off the UI thread, and both client and server updates automatically remove completed download and staging files.
...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.38` requires database schema version `12`. If the database is newer than the client, the connection screen checks the public release channel and offers to install a compatible client before any workspace connection is attempted.

### Coordinated 0.5.38 upgrade
...
The 0.5.38 client and service require the schema-version-12 procedure set. Upgrading from an earlier schema requires the matching SQL deployment before either application is updated.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.38
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.38
~~~
