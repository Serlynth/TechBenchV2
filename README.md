...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `5.0.1` requires database schema version `7`, including the service-only WHD and Sage ingestion contracts and restricted Admin preview boundary. Version 5.0.1 introduces no database migration.

### Coordinated 5.0.1 upgrade
...
5. Open Server Manager as a TechBench Admin and configure the SQL connection, WHD endpoint, authentication mode, service username, five-minute schedule, server Sage DSN/username, and bulk AD-to-WHD technician mappings. Install the 5.0.1 client on workstations; each technician configures their personal posting credentials, Sage Activity Item ID, and local workstation options.
...
Do not deploy only one side. The 5.0.1 client and service require schema version 7; earlier alpha clients retain obsolete organization-sync controls in their Settings page.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 5.0.1
.\scripts\Publish-TechBenchServer.ps1 -Version 5.0.1
~~~
