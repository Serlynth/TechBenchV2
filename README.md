...
Current milestone: `0.5.30` - TechBench V2 uses the `0.5.x` development release line; "V2" remains the product generation and is assumed. Admin Center can send one alert or safe sign-out request to multiple active clients and opens a live per-user response window.
...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.30` requires database schema version `11`. If the database is newer than the client, the connection screen checks the public release channel and offers to install a compatible client before any workspace connection is attempted.

### Coordinated 0.5.30 upgrade
...
The 0.5.30 client and service require the schema-version-11 procedure set. No database change is required when upgrading from 0.5.24 through 0.5.29.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.30
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.30
~~~
