...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.16` requires database schema version `8`, including encrypted Credentials storage, the service-only import contract, user reveal procedures, and the Admin-only configuration/request boundary. If the database is newer than the client, the connection screen checks the public release channel and offers to install a compatible client before any workspace connection is attempted.

### Coordinated 0.5.16 upgrade
...
Do not deploy only one side. The 0.5.16 client and service require the schema-version-8 procedure set.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.16
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.16
~~~
