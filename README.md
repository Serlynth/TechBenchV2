...
Current application version: `1.2.16`.
...
After a verified WHD post, the entry remains editable until it is posted to Sage. Saving compares the local work note, the exact WHD TechNote, and the last verified snapshot: one-sided changes synchronize automatically, while competing changes require an explicit choice. **Sync WHD Note** can pull the WHD version on demand. Updates use `PUT` against the tracked TechNote ID and never fall back to creating another note. Sage posting first verifies this WHD synchronization when the entry has a ticket.
