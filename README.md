...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.7` requires database schema version `8`, including the encrypted FireDrill storage, service-only import contract, user reveal/copy procedures, and Admin-only configuration/request boundary.

### Coordinated 0.5.7 upgrade
...
5. Grant `CSRI\TechBench_Sync` read access to both the FireDrill share and workbook. In Server Manager, open **FireDrill**, confirm the UNC path and 4:00 AM schedule, enter the workbook open password under **Workbook open password**, select **Save / Rotate**, then select **Sync now**. Install the 0.5.7 client on workstations.
...
Do not deploy only one side. The 0.5.7 client and service require the schema-version-8 procedure set.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.7
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.7
~~~
