...
Current milestone: `0.5.2` - TechBench V2 uses the `0.5.x` development release line; “V2” remains the product generation and is assumed. The Server Manager organizes Service, SQL Server, Web Help Desk, Sage 50, and Updates into dedicated tabs; WHD connection settings and user mappings have their own sections; and the activity feed is newest-first. Server-owned Sage customer reads retain the established client behavior of ignoring unusable blank-ID rows and using the customer ID when a display name is blank, while SQL still rejects duplicate IDs and over-length values. Existing installations preserve their service identity, SQL configuration, and protected WHD/Sage secrets during update or repair. Installations that received mistakenly numbered `5.0.1` or `5.0.2` update once to the `5.0.3` transition bridge and then onto the current `0.5.x` line.
...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.2` requires database schema version `7`, including the service-only WHD and Sage ingestion contracts and restricted Admin preview boundary. Version 0.5.2 introduces no database migration.

### Coordinated 0.5.2 upgrade
...
5. Open Server Manager as a TechBench Admin and configure the SQL connection, WHD endpoint, authentication mode, service username, five-minute schedule, server Sage DSN/username, and bulk AD-to-WHD technician mappings. Install the 0.5.2 client on workstations; each technician configures their personal posting credentials, Sage Activity Item ID, and local workstation options.
...
Do not deploy only one side. The 0.5.2 client and service require schema version 7; earlier alpha clients retain obsolete organization-sync controls in their Settings page.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.2
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.2
~~~
