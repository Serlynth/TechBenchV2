...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.9` requires database schema version `8`, including the encrypted FireDrill storage, service-only import contract, user reveal/copy procedures, and Admin-only configuration/request boundary.

### Coordinated 0.5.9 upgrade
...
Do not deploy only one side. The 0.5.9 client and service require the schema-version-8 procedure set.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.9
