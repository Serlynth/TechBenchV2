...
Current milestone: `0.5.40` - TechBench V2 uses the `0.5.x` development release line; "V2" remains the product generation and is assumed. Server Manager now offers a technician-only WHD synchronization from User Mappings, and current-account recovery honors WHD's session-specific WebObjects instance so active Admin accounts omitted by the normal technician list can still be mapped.
...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.40` requires database schema version `12`. If the database is newer than the client, the connection screen checks the public release channel and offers to install a compatible client before any workspace connection is attempted.

### Coordinated 0.5.40 upgrade
...
The 0.5.40 client and service require the matching schema-version-12 procedure set. Even when schema 12 is already installed, reapply the 0.5.40 SQL deployment so the technician-only request type and current-account recovery are installed.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.40
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.40
~~~
