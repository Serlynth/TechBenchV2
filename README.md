...
Current milestone: `0.5.39` - TechBench V2 uses the `0.5.x` development release line; "V2" remains the product generation and is assumed. WHD administrators omitted by the normal technician list are recovered from current-account and assigned-ticket data, and Server Manager distinguishes active synchronization work from waiting work.
...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.39` requires database schema version `12`. If the database is newer than the client, the connection screen checks the public release channel and offers to install a compatible client before any workspace connection is attempted.

### Coordinated 0.5.39 upgrade
...
The 0.5.39 client and service require the matching schema-version-12 procedure set. Even when schema 12 is already installed, reapply the 0.5.39 SQL deployment so assigned WHD administrators omitted by the Techs collection are recovered for user mapping.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.39
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.39
~~~
