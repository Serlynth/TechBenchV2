...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.3` requires database schema version `7`, including the service-only WHD and Sage ingestion contracts, automatic client-matching procedures, and restricted Admin preview boundary. Version 0.5.3 introduces no database migration, but its SQL deployment must be reapplied before updating the service.

### Coordinated 0.5.3 upgrade
...
5. Open Server Manager as a TechBench Admin and configure the SQL connection, WHD endpoint, authentication mode, service username, five-minute schedule, server Sage DSN/username, and bulk AD-to-WHD technician mappings. Install the 0.5.3 client on workstations; each technician configures their personal posting credentials, Sage Activity Item ID, and local workstation options.
...
Do not deploy only one side. The 0.5.3 client and service require the refreshed schema-version-7 procedure set; earlier alpha clients retain obsolete organization-sync controls in their Settings page.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.3
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.3
~~~
