...
Current milestone: `0.5.25` - TechBench V2 uses the `0.5.x` development release line; "V2" remains the product generation and is assumed. The Admin-only Admin Center queues server-owned WHD and Sage synchronizations, shows active TechBench clients, sends notices that accept a written response, and requests a safe TechBench-only sign-out. Its unified refresh updates both synchronization cards, active clients, and recent responses.
...
The desktop application and Server Manager check schema compatibility and refuse an incompatible deployment. Version `0.5.25` requires database schema version `11`. If the database is newer than the client, the connection screen checks the public release channel and offers to install a compatible client before any workspace connection is attempted.

### Coordinated 0.5.25 upgrade
...
The 0.5.25 client and service require the schema-version-11 procedure set. No database change is required when upgrading from 0.5.24.
...
.\scripts\Publish-TechBenchRelease.ps1 -Version 0.5.25
.\scripts\Publish-TechBenchServer.ps1 -Version 0.5.25
~~~
