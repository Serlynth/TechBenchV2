...
param(
    [Security.SecureString]$SageCredential,
...
if ($null -eq $SageCredential) {
    $SageCredential = Read-Host 'Sage ODBC password for server customer synchronization' -AsSecureString
}

if ($PSCmdlet.ShouldProcess('TechBench Sync Service', 'Replace the protected server-local Sage ODBC credential')) {
    $bstr = [IntPtr]::Zero
    $plainText = $null
    try {
        $bstr = [Runtime.InteropServices.Marshal]::SecureStringToBSTR($SageCredential)
        $plainText = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($bstr)
        if ([string]::IsNullOrWhiteSpace($plainText)) {
            throw 'The Sage ODBC credential cannot be empty.'
        }

        $startInfo = New-Object Diagnostics.ProcessStartInfo
        $startInfo.FileName = $executablePath
        $startInfo.Arguments = '--set-sage-secret'
