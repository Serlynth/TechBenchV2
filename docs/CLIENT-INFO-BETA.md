...
3. Copy the useful, cleaned information from the client's current workbook into
   the matching tabs: Locations, People, Equipment, Servers & Infrastructure,
   Connection & Internet, Wi-Fi, Applications & Cloud, Domains & Email,
...
The current workbook format is `TB-CI-6`. It intentionally hides technical row
keys, uses ordinary worksheet labels, and separates technology information by
category. Servers & Infrastructure, Connection & Internet, Wi-Fi, and
Applications & Cloud now include category-specific networking columns and
accept optional `Custom:` columns. Workbooks generated by the earlier
`TB-CI-5`, `TB-CI-4`, `TB-CI-3`, `TB-CI-2`, and `TB-CI-1` betas remain
importable.
