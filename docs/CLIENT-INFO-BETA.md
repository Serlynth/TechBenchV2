...
4. Do not change the internal client ID or reuse the workbook for another
   client. Enter the primary AD password beside its user. For Microsoft 365,
   choose whether the AD login is reused; if it is not, enter the separate
   Microsoft 365 username and password. Enter the primary login beside each
   system or service. Use Passwords for additional or standalone credentials.
   Completed workbooks contain plaintext secrets until import, so secure and
   remove the files according to policy.
...
sheet provides AD Username and AD Password columns with the same behavior, plus
a **Microsoft 365 Uses AD Login** choice and separate Microsoft 365 username
and password columns for users whose cloud login is not synchronized with AD.
Workbooks generated by `TB-CI-8` through `TB-CI-1` remain importable.
