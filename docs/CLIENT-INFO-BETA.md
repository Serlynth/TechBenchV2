...
3. Copy the useful, cleaned information from the client's current workbook into
   ...
   Network & Internet, Applications & Cloud, Domains & Email, Backup &
   Security, Vendors & Services, Passwords, and Other Info. Leave tabs or
   fields that do not apply blank. Antivirus and EDR belong under Backup &
   Security.
...
The beta.7 workbook format is `TB-CI-3`. It intentionally hides technical row
keys, uses ordinary worksheet labels, and separates technology information by
category. Workbooks generated by the earlier `TB-CI-2` and `TB-CI-1` betas
remain importable.
