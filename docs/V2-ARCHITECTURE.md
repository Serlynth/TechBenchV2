...
TechBench V2 `2.0.0-alpha.12` implements the conversion from TechBench 1.x's local SQLite design to a shared SQL Server design, including an owner-scoped V1 migration contract, a dedicated Windows service for organization-wide WHD and Sage customer synchronization, an Admin-only read-only user-preview boundary, and a server-local administrator GUI with shared synchronization configuration, a reliable launcher, notification-area behavior, and controlled service updates. Alpha.12 also fixes GitHub multi-release enumeration under Windows PowerShell 5.1.
...
Routine updates treat the Manager script, PowerShell launcher, consoleless shim, and icon as one companion set. Their staged, installed, and rollback paths are journaled before replacement, and interrupted-update recovery classifies every companion before committing or rolling back. On normal startup the alpha.12 Manager also verifies these files against the installed service package and repairs the Start Menu shortcut. Alpha.9 requires a one-time extracted-package bootstrap because its PowerShell 5.1 updater nests the GitHub release list and finds no candidates; alpha.12 captures the response before enumeration and can discover later releases normally. Shared configuration and machine-protected service credentials are not part of this repair.
...
The schema-version-7 database, alpha.12 client, and alpha.12 sync service are a coordinated cutover. Have the DBA back up and verify the database, install the service and both protected credentials, configure shared WHD/Sage values in Server Manager, install the matching client, and run smoke tests as one planned operation. Do not leave mixed alpha clients in normal use.
...
Until those checks pass, alpha.12 is an implementation candidate, not a production release.

## V1 data migration

Installing alpha.12 does not automatically import V1 data; each authenticated user explicitly uses **Settings > Import V1 Database...** and confirms an import preview for their own account. This is separate from the Admin-only login preview.
