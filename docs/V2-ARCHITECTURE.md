...
TechBench V2 `0.5.5` implements the conversion from TechBench 1.x's local SQLite design to a shared SQL Server design, including an owner-scoped V1 migration contract, a dedicated Windows service for organization-wide WHD and Sage customer synchronization, an Admin-only read-only user-preview boundary, a compiled server-local administrator GUI, and a self-contained one-click server installer. Normal Ticket List reads are scoped to the mapped WHD technician or synchronized groups even for Admins; Server Manager supports bulk AD-to-WHD mapping; work-entry tags and Sage activity settings are owner-scoped; and high-confidence WHD-to-Sage client matching, including safe numbered multi-location families, runs centrally after either source snapshot.
...
This milestone enforces those rules through the stored-procedure boundary, the absence of direct table permissions, `tb_security.WhdTicketAccessPolicy`, and the V7 read-only preview execution context. Normal application ticket procedures filter WHD rows to the mapped technician or technician group for every user, including Admins, and the row-level policy applies insert/update block predicates. A valid preview target scopes reads to the selected technician, while the dedicated sync service and database owners retain the access required for their roles.
...
The legacy sync-operator role never grants a non-Admin permission to start or apply shared WHD work. Admins can queue work but cannot call service apply procedures. Every normal client session sees only WHD tickets directly assigned to its mapped technician or assigned to one of that technician's synchronized groups. Admins use the explicit read-only user preview to inspect another technician rather than receiving an organization-wide Ticket List.
...
The schema-version-7 database, 0.5.5 client, and 0.5.5 sync service are a coordinated cutover. Have the DBA back up and verify the database, reapply the matching SQL deployment, configure shared WHD/Sage values in Server Manager, install the 0.5.5 client, and run smoke tests as one planned operation. Do not leave mixed alpha clients in normal use.
...
Until those checks pass, 0.5.5 is an implementation candidate, not a production release.

## V1 data migration

Installing 0.5.5 does not automatically import V1 data; each authenticated user explicitly uses **Settings > Import V1 Database...** and confirms an import preview for their own account. This is separate from the Admin-only login preview.
