...
TechBench V2 `2.0.0-alpha.13` implements the conversion from TechBench 1.x's local SQLite design to a shared SQL Server design, including an owner-scoped V1 migration contract, a dedicated Windows service for organization-wide WHD and Sage customer synchronization, an Admin-only read-only user-preview boundary, and a server-local administrator GUI with shared synchronization configuration, a reliable launcher, notification-area behavior, and controlled service updates. Alpha.13 includes corrected GitHub multi-release enumeration and SQL connection-string construction under Windows PowerShell 5.1.
...
TechBench Server Manager is a server-local administrator process, not a network service. It is the sole product UI for organization-wide WHD/Sage synchronization configuration and manual sync requests. It connects to SQL Server with the elevated operator's Windows identity, requires the TechBench Admin role, and uses only the existing stored-procedure boundary; external-system secrets remain machine-protected outside SQL. Its Start Menu shortcut invokes a consoleless VBScript shim, which starts the checked PowerShell launcher hidden, in STA mode, and with elevation. The launcher selects 64-bit Windows PowerShell even from an unusual 32-bit caller. Startup failures are shown to the operator. Elevated failures are logged only under the protected Manager data directory; failures detected before elevation may use per-user fallbacks. They are not allowed to disappear with a transient console window.
...
Routine updates treat the Manager script, PowerShell launcher, consoleless shim, and icon as one companion set. Their staged, installed, and rollback paths are journaled before replacement, and interrupted-update recovery classifies every companion before committing or rolling back. On normal startup the alpha.13 Manager also verifies these files against the installed service package and repairs the Start Menu shortcut. Alpha.9 requires a one-time extracted-package bootstrap because its PowerShell 5.1 updater nests the GitHub release list and finds no candidates; alpha.13 captures the response before enumeration and uses canonical connection-string keywords for schema verification. Shared configuration and machine-protected service credentials are not part of this repair.
...
The schema-version-7 database, alpha.13 client, and alpha.13 sync service are a coordinated cutover. Have the DBA back up and verify the database, install the service and both protected credentials, configure shared WHD/Sage values in Server Manager, install the matching client, and run smoke tests as one planned operation. Do not leave mixed alpha clients in normal use.
...
Until those checks pass, alpha.13 is an implementation candidate, not a production release.

## V1 data migration

Installing alpha.13 does not automatically import V1 data; each authenticated user explicitly uses **Settings > Import V1 Database...** and confirms an import preview for their own account. This is separate from the Admin-only login preview.
