...
The publisher creates the directly runnable `dist\TechBenchServerSetup.exe` and its SHA-256 sidecar. The setup executable embeds the complete verified `TechBenchSyncService-0.5.25-win-x64.zip` payload, including the x64 service, x86 Sage worker, compiled Server Manager, configuration template, runbook, credential helpers, and matching SQLCMD deployment. Do not place any external-system secret in the package or in `appsettings.json`.

The same command also creates the directly downloadable
`dist\TechBenchV2-SQLServer2016-0.5.25.sql` and its checksum. After the
matching client publisher has created GitHub release `v0.5.25`, attach
...
```powershell
$sql = '.\TechBenchV2-SQLServer2016-0.5.25.sql'
...
`https://github.com/Serlynth/TechBenchV2-Releases/releases/download/v0.5.25/TechBenchServerSetup.exe`
