...
public sealed class WhdRestClientTests
{
    ...
    [Fact]
    public void TechNoteApiIsAppendOnly()
    {
        var methods = typeof(WhdRestClient)
            .GetMethods()
            .Select(static method => method.Name)
            .ToHashSet(StringComparer.Ordinal);

        Assert.Contains(nameof(WhdRestClient.PostTicketNoteAsync), methods);
        Assert.Contains(nameof(WhdRestClient.UploadTechNoteImagesAsync), methods);
        Assert.DoesNotContain("GetTechNoteAsync", methods);
        Assert.DoesNotContain("UpdateTechNoteAsync", methods);
        Assert.DoesNotContain("DeleteTechNoteAsync", methods);
    }
